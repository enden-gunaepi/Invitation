...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        $defaultSubscription = $this->clientPackageService->getDefaultUsableSubscription((int) $user->id);
        ...
        $invitations = Cache::remember("{$cachePrefix}:invitations:v1", now()->addSeconds(30), function () use ($user) {
            return Invitation::query()
                ...
                ->select(['id', 'user_id', 'template_id', 'package_id', 'client_package_subscription_id', 'title', 'status', 'event_date', 'view_count', 'created_at'])
                ->latest()
                ->take(5)
                ->get();
        });
        ...
        $latestInvitation = Cache::remember("{$cachePrefix}:latest_invitation:v1", now()->addSeconds(30), function () use ($user) {
            return Invitation::query()
                ...
                ->withCount(['guests', 'photos'])
                ->select(['id', 'user_id', 'package_id', 'client_package_subscription_id', 'title', 'venue_name', 'status', 'created_at'])
                ->latest()
                ->first();
        });

        $hasActivePackage = $defaultSubscription !== null;
        $onboarding = $this->buildOnboardingData($latestInvitation, $hasActivePackage, $defaultSubscription?->id);
        $upsell = $this->buildUpsellData($latestInvitation, $user->id);

        return view('client.dashboard.index', compact('stats', 'invitations', 'onboarding', 'upsell', 'hasActivePackage', 'defaultSubscription'));
    }

    private function buildOnboardingData(?Invitation $invitation, bool $hasActivePackage, ?int $defaultSubscriptionId): array
    {
        if (!$hasActivePackage) {
            return [
                'progress' => 0,
        ...
        if (!$invitation) {
            return [
                ...
                'next_url' => $defaultSubscriptionId
                    ? route('client.invitations.create', ['subscription_id' => $defaultSubscriptionId])
    ...
    private function buildUpsellData(?Invitation $invitation, int $userId): ?array
    {
        ...
        $invUsed = $invitation->client_package_subscription_id
            ? Invitation::where('client_package_subscription_id', $invitation->client_package_subscription_id)->count()
            : Invitation::where('user_id', $userId)->where('package_id', $package->id)->count();
