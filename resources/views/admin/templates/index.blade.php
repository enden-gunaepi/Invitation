...
<div class="flex items-center justify-between mb-6">
    ...
    <a href="{{ route('admin.templates.create') }}" class="btn-primary text-sm">
...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
    ...
    <div class="card overflow-hidden group">
        <div class="h-40 bg-gradient-to-br from-indigo-500/20 to-purple-500/20 flex items-center justify-center relative">
            @if($template->thumbnail)
                <img src="{{ asset('storage/' . $template->thumbnail) }}" class="w-full h-full object-cover" alt="{{ $template->name }}">
            @else
                <i class="fas fa-palette text-4xl text-indigo-400/40"></i>
            @endif
            @if($template->is_premium)
                <span class="absolute top-3 right-3 bg-amber-500/20 text-amber-400 text-xs font-bold px-3 py-1 rounded-full">
        ...
        <div class="p-5">
            ...
            <div class="flex items-center gap-2">
                <a href="{{ route('admin.templates.edit', $template) }}" class="btn-outline text-xs py-2 px-4 flex-1 text-center">Edit</a>
                <form method="POST" action="{{ route('admin.templates.destroy', $template) }}" onsubmit="return confirm('Hapus template?')">
                    @csrf @method('DELETE')
