...
<div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
    <form class="flex items-center gap-3" method="GET">
        <div class="relative">
            ...
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari undangan..." class="form-input pl-10 w-64">
        </div>
...
<div class="card overflow-hidden">
    <table class="data-table">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td class="text-right">
                    <div class="flex items-center justify-end gap-2">
                        <a href="{{ route('admin.invitations.show', $inv) }}" class="text-indigo-400 hover:text-indigo-300 text-sm" title="Detail">
                            <i class="fas fa-eye"></i>
                        </a>
                        @if($inv->status === 'pending')
                        <form method="POST" action="{{ route('admin.invitations.approve', $inv->id) }}" class="inline">
                            @csrf @method('PATCH')
                            <button class="text-emerald-400 hover:text-emerald-300 text-sm" title="Approve"><i class="fas fa-check"></i></button>
                        </form>
                        <form method="POST" action="{{ route('admin.invitations.reject', $inv->id) }}" class="inline"
                            ...
                            <button class="text-red-400 hover:text-red-300 text-sm" title="Reject"><i class="fas fa-times"></i></button>
                        </form>
                        @endif
                    </div>
                </td>
            ...
            @empty
            <tr><td colspan="7" class="text-center py-8 text-slate-500">Tidak ada undangan</td></tr>
            @endforelse
        </tbody>
    </table>
</div>
