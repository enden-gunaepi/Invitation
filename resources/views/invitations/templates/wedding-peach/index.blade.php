    ...
    <style>
        ...
        * {
        ...
        @keyframes kenburn {
            ...
            to {
                transform: scale(1) translate3d(-0.8%, -0.8%, 0);
            }
        }
        ...
        .open-btn:hover {
            ...
            box-shadow: 0 8px 24px rgba(232, 149, 112, .3);
        }
        ...
        @keyframes pulseRing {
            ...
            50% {
                box-shadow: 0 0 0 9px rgba(255, 255, 255, 0.05);
            }
        }
...
<body>
    ...
    @if ($bgSlides->count())
        <div class="bg-slideshow" id="bgSlideshow">
            @foreach ($bgSlides as $slidePath)
                <img src="{{ asset('storage/' . $slidePath) }}" class="bg-slide {{ $loop->first ? 'active' : '' }}"
                    alt="Background" loading="{{ $loop->first ? 'eager' : 'lazy' }}" decoding="async"
    ...
    <section class="cover" id="cover">
        <div class="peach-frame" data-aos="zoom-in" data-aos-duration="900">
            ...
            @endif
            <button class="open-btn" onclick="openInvitation()"><i class="fas fa-envelope-open-text mr-2"></i>Buka
                Undangan</button>
        </div>
    </section>
    ...
    <main class="content" id="content">
        ...
        <section class="section section-alt">
            ...
            @php
                $coupleBg =
                    $invitation->photos->first()->file_path ?? ($invitation->groom_photo ?? $invitation->bride_photo);
            @endphp
            <div class="couple-grid">
                <div class="couple-stage"
                    style="{{ $coupleBg ? "background-image: url('" . asset('storage/' . $coupleBg) . "');" : 'background: linear-gradient(180deg, #4d443e 0%, #2f2824 100%);' }}">
                    <div class="couple-card groom" data-aos="fade-right" data-aos-delay="200">
                        <div class="couple-photo-ring">
                            @if ($invitation->groom_photo)
                                <img src="{{ asset('storage/' . $invitation->groom_photo) }}" class="couple-photo"
                                    alt="Groom" loading="lazy" decoding="async">
                            @else
                                <div class="couple-photo flex items-center justify-center font-bold"
                                    style="background:var(--peach-100); color:var(--peach-700);">
                        ...
                        @if ($invitation->groom_parent_name)
                            <div class="identity-parent">{{ $invitation->groom_parent_name }}</div>
                        @endif
                        <div class="social-links mt-2">
                            @if ($invitation->groom_instagram)
                                <a class="social-btn"
                                    href="{{ str_starts_with($invitation->groom_instagram, 'http') ? $invitation->groom_instagram : 'https://instagram.com/' . ltrim($invitation->groom_instagram, '@') }}"
                                    target="_blank"><i class="fab fa-instagram"></i> Instagram</a>
                            @endif
                            @if ($invitation->groom_facebook)
                                <a class="social-btn" href="{{ $invitation->groom_facebook }}" target="_blank"><i
                    ...
                    <div class="couple-card bride" data-aos="fade-left" data-aos-delay="250">
                        <div class="couple-photo-ring">
                            @if ($invitation->bride_photo)
                                <img src="{{ asset('storage/' . $invitation->bride_photo) }}" class="couple-photo"
                                    alt="Bride" loading="lazy" decoding="async">
                            @else
                                <div class="couple-photo flex items-center justify-center font-bold"
                                    style="background:var(--peach-100); color:var(--peach-700);">
                        ...
                        @if ($invitation->bride_parent_name)
                            <div class="identity-parent">{{ $invitation->bride_parent_name }}</div>
                        @endif
                        <div class="social-links mt-2">
                            @if ($invitation->bride_instagram)
                                <a class="social-btn"
                                    href="{{ str_starts_with($invitation->bride_instagram, 'http') ? $invitation->bride_instagram : 'https://instagram.com/' . ltrim($invitation->bride_instagram, '@') }}"
                                    target="_blank"><i class="fab fa-instagram"></i> Instagram</a>
                            @endif
                            @if ($invitation->bride_facebook)
                                <a class="social-btn" href="{{ $invitation->bride_facebook }}" target="_blank"><i
                                        class="fab fa-facebook"></i> Facebook</a>
                            @endif
                        </div>
                    </div>
        ...
        <section class="section">
            ...
            <div class="countdown" id="countdown"
                ...
                <div class="cd-item">
                    <div class="cd-num" id="cd-hours">0</div>
                    <div class="cd-lbl">Jam</div>
                </div>
                <div class="cd-item">
                    <div class="cd-num" id="cd-minutes">0</div>
                    <div class="cd-lbl">Menit</div>
                </div>
                <div class="cd-item">
                    <div class="cd-num" id="cd-seconds">0</div>
        ...
        <section class="section section-alt">
            ...
            <div class="timeline">
                @foreach ($rundownItems as $item)
                    <div class="timeline-item {{ $loop->odd ? 'left' : 'right' }}" data-aos="fade-up"
                        data-aos-delay="{{ 170 + $loop->index * 70 }}">
            ...
            <div style="margin-top:1rem;" data-aos="fade-up" data-aos-delay="260">
                <a href="{{ $invitation->google_calendar_url }}" target="_blank" class="open-btn"
                ...
                <div class="story-grid">
                    @foreach ($invitation->loveStories as $story)
                        <div class="wish-item" data-aos="fade-up" data-aos-delay="{{ 180 + $loop->index * 70 }}">
                            @if ($story->photo_path)
                                <img src="{{ asset('storage/' . $story->photo_path) }}" alt="{{ $story->title }}"
                                    class="story-photo" loading="lazy" decoding="async">
                            @endif
                            @if ($story->year)
                                <div class="wish-name">{{ $story->year }}</div>
                            @endif
                            <div class="wish-name">{{ $story->title }}</div>
                            @if ($story->description)
                                <div class="wish-message">{{ $story->description }}</div>
                            @endif
                        </div>
                    @endforeach
                </div>
                ...
                <div class="gallery">
                    @foreach ($invitation->photos as $photo)
                        <img src="{{ asset('storage/' . $photo->file_path) }}"
                            alt="{{ $photo->caption ?? 'Gallery Photo' }}" loading="lazy" decoding="async"
        ...
        @if ($invitation->google_maps_url || $invitation->venue_address)
            <section class="section">
                ...
                <div class="maps" data-aos="fade-up" data-aos-delay="220">
                    <div id="leafletMap" class="leaflet-map" data-lat="{{ $invitation->venue_lat }}"
                        data-lng="{{ $invitation->venue_lng }}" data-url="{{ $invitation->maps_deep_link }}"
                ...
                <div style="margin-top:1rem;display:flex;gap:.5rem;justify-content:center;flex-wrap:wrap;">
                    <a href="{{ $invitation->maps_deep_link }}" target="_blank" class="open-btn"
                        style="display:inline-block;text-decoration:none;">
                        <i class="fas fa-location-dot mr-2"></i>Buka Google Maps
                    </a>
                    <a href="https://waze.com/ul?q={{ urlencode($invitation->venue_name . ' ' . $invitation->venue_address) }}"
                        target="_blank" class="open-btn"
                        style="display:inline-block;text-decoration:none;background:linear-gradient(135deg,#80d4ff,#3ba4e8);border-color:#3ba4e8;">
        ...
        @if ($invitation->livestream_enabled && $invitation->livestream_url)
            @php
                ...
                $ytWatch = preg_match('~(?:youtube\.com/watch\?v=|youtu\.be/)([^&?/]+)~', $streamUrl, $m)
                    ? $m[1]
                    : null;
            ...
            <section class="section section-alt">
                ...
                <h2 class="section-title" data-aos="fade-up" data-aos-delay="100">
                    {{ $invitation->livestream_label ?: 'Live Streaming' }}</h2>
                <div class="divider" data-aos="fade-up" data-aos-delay="150"></div>
                @if ($embedUrl)
                    <div class="maps" data-aos="fade-up" data-aos-delay="220">
                        <iframe src="{{ $embedUrl }}" width="100%" height="290" style="border:0;"
                            allowfullscreen loading="lazy"></iframe>
                    </div>
                @endif
                <a href="{{ $invitation->livestream_url }}" target="_blank" class="open-btn"
            ...
            <div class="panel" data-aos="fade-up" data-aos-delay="220" style="max-width:560px;margin:0 auto;">
                <form method="POST" action="{{ route('invitation.rsvp', $invitation->slug) }}">
                    ...
                    @if (isset($guest))
                        ...
                        <p class="wish-message" style="margin-bottom:.7rem;">Halo,
                    ...
                    <input type="number" name="pax" class="input" value="1" min="1"
                        max="10" placeholder="Jumlah tamu">
                    <textarea name="message" class="textarea" placeholder="Pesan (opsional)"></textarea>
                    <button type="submit" class="btn"><i class="fas fa-paper-plane mr-2"></i>Kirim
                        Konfirmasi</button>
                </form>
            </div>
        ...
        <section class="section" id="wishes">
            ...
            <div class="panel" data-aos="fade-up" data-aos-delay="220" style="max-width:560px;margin:0 auto;">
                <form method="POST" action="{{ route('invitation.wish', $invitation->slug) }}">
                    @csrf
                    <input type="text" name="name" class="input" placeholder="Nama Anda"
        ...
        @if ($invitation->bank_name || $invitation->gift_address || $invitation->bankAccounts->count())
            <section class="section section-alt">
                ...
                <div class="panel" style="max-width:560px;margin:0 auto;" data-aos="fade-up" data-aos-delay="220">
                    <div class="gift-tabs">
                        <button type="button" class="gift-tab-btn active" data-target="gift-digital">Amplop
                            Digital</button>
                    ...
                    <div id="gift-digital" class="gift-pane active">
                        @if ($invitation->bankAccounts->count())
                            @foreach ($invitation->bankAccounts as $acc)
                                <div class="wish-item mb-2">
                                    ...
                                    <div class="wish-message">{{ $acc->account_number }} a/n {{ $acc->account_name }}
                                    </div>
                                </div>
                            @endforeach
                        @elseif($invitation->bank_name)
                            <div class="wish-item">
                                ...
                                <div class="wish-message">{{ $invitation->bank_account_number }} a/n
                                    {{ $invitation->bank_account_name }}</div>
                            </div>
                        @else
