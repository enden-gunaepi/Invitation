...
<head>
    ...
    <link
        href="https://fonts.googleapis.com/css2?family=Cormorant+Garamond:wght@500;700&family=Nunito:wght@300;400;600&display=swap"
        rel="stylesheet">
    <style>
        body {
...
<body>
    ...
    @endphp
    @if($isDemoMode)
        <div class="fixed left-3 top-3 z-[90] bg-amber-400 text-slate-900 text-xs font-bold px-3 py-1 rounded-full">Demo
            Mode</div>
    @endif

    <section id="cover"
        class="relative min-h-screen flex items-center justify-center text-center px-6 py-20 transition-opacity duration-700">
        ...
        <div class="relative z-10 max-w-2xl">
            ...
            <button type="button" id="openInvite"
                class="mt-8 bg-rose-700 hover:bg-rose-600 px-6 py-3 rounded-full text-sm font-semibold">
                Buka Undangan
            </button>
        </div>
    </section>
    ...
    <main id="content" class="opacity-0 pointer-events-none transition-opacity duration-700">
        <div id="galleryBg" class="fixed inset-0 -z-10">
            ...
            <div class="absolute inset-0 bg-gradient-to-b from-slate-950/60 via-slate-900/25 to-slate-950/75"></div>
        </div>
        ...
        <section class="max-w-6xl mx-auto px-6 pb-16">
            <div class="grid md:grid-cols-2 gap-6">
                <article class="glass-card rounded-3xl p-7 text-center">
                    @if($bridePhoto)
                        <img src="{{ $bridePhoto }}" alt="Foto mempelai wanita"
                            class="w-36 h-36 mx-auto rounded-full object-cover border-2 border-white/70 mb-4">
                    @endif
                    ...
                    @if($invitation->bride_instagram)
                        <a href="{{ $invitation->bride_instagram }}" target="_blank"
                            class="inline-block mt-5 text-sm border border-white/40 px-4 py-2 rounded-full hover:bg-white/15 transition">@Instagram</a>
                    @endif
                </article>
                <article class="glass-card rounded-3xl p-7 text-center">
                    @if($groomPhoto)
                        <img src="{{ $groomPhoto }}" alt="Foto mempelai pria"
                            class="w-36 h-36 mx-auto rounded-full object-cover border-2 border-white/70 mb-4">
                    @endif
            ...
            <article class="glass-card rounded-3xl p-8 text-center">
                ...
                <div class="grid grid-cols-4 gap-3 mb-4">
                    <div class="bg-white/10 rounded-xl p-4">
                        <p id="days" class="text-2xl font-bold">0</p>
                ...
                <a href="#rsvp"
                    class="inline-block mt-3 bg-rose-700 hover:bg-rose-600 px-5 py-2 rounded-full text-sm font-semibold">Konfirmasi
                    Kehadiran</a>
            </article>
        </section>
        ...
        @if($mapsEmbed)
            <section class="max-w-6xl mx-auto px-6 pb-16">
                <article class="glass-card rounded-3xl p-8 text-center">
                    ...
                    @if($mapsUrl)
                        <a href="{{ $mapsTrackedUrl }}" target="_blank"
                            class="inline-block mt-5 bg-rose-700 hover:bg-rose-600 px-5 py-3 rounded-full text-sm font-semibold">Gunakan
                            Google Maps</a>
                    @endif
                </article>
            </section>
                ...
                <div class="relative max-w-4xl mx-auto">
                    ...
                    @foreach($invitation->events as $event)
                        <div class="mb-10 flex flex-col md:flex-row items-center">
                            <div
                                class="md:w-1/2 md:pr-8 text-center md:text-right {{ $loop->even ? 'order-2 md:order-1' : '' }}">
                                @if($loop->odd)
                ...
                <article class="glass-card rounded-3xl p-8">
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                        @foreach($invitation->photos as $photo)
                            <img src="{{ asset('storage/' . $photo->file_path) }}" alt="Galeri foto" loading="lazy"
                                decoding="async" data-lightbox="gallery"
                                class="w-full h-44 object-cover rounded-xl cursor-zoom-in">
                        @endforeach
                    </div>
                </article>
            </section>
        ...
        <section id="rsvp" class="max-w-6xl mx-auto px-6 pb-16">
            <div class="grid md:grid-cols-2 gap-6">
                <article class="glass-card rounded-3xl p-7">
                    ...
                    <form method="POST" action="{{ $isDemoMode ? '#' : route('invitation.rsvp', $invitation->slug) }}"
                        class="space-y-3 demo-form">
                        @csrf
                        <input type="text" name="name" value="{{ $guest->name ?? '' }}" placeholder="Nama"
                            class="w-full px-4 py-3 rounded-lg text-slate-900" required>
                        <input type="text" name="phone" placeholder="Nomor HP"
                            class="w-full px-4 py-3 rounded-lg text-slate-900">
                        ...
                        <input type="number" min="1" max="10" name="pax" value="1"
                            class="w-full px-4 py-3 rounded-lg text-slate-900" required>
                        <textarea name="message" rows="3" placeholder="Pesan RSVP"
                            class="w-full px-4 py-3 rounded-lg text-slate-900"></textarea>
                        @if(!empty($guest?->id))
                            <input type="hidden" name="guest_id" value="{{ $guest->id }}">
                        @endif
                        <button type="submit"
                            class="w-full bg-rose-700 hover:bg-rose-600 px-4 py-3 rounded-lg font-semibold">{{ $personalization['cta'] }}</button>
                    </form>
                    <div class="mt-4 space-y-2 max-h-64 overflow-y-auto">
                        ...
                        <div class="flex items-center justify-between pt-1 list-rsvp-pager">
                            <button type="button" class="text-xs bg-white/20 hover:bg-white/30 px-3 py-1 rounded-full"
                ...
                <article class="glass-card rounded-3xl p-7">
                    ...
                    <form method="POST" action="{{ $isDemoMode ? '#' : route('invitation.wish', $invitation->slug) }}"
                        class="space-y-3 demo-form">
                        @csrf
                        <input type="text" name="name" value="{{ $guest->name ?? '' }}" placeholder="Nama"
                            class="w-full px-4 py-3 rounded-lg text-slate-900" required>
                        <textarea name="message" rows="4" placeholder="Ucapan untuk mempelai"
                            class="w-full px-4 py-3 rounded-lg text-slate-900" required></textarea>
                        <button type="submit"
                            class="w-full bg-rose-700 hover:bg-rose-600 px-4 py-3 rounded-lg font-semibold">Kirim
                            Ucapan</button>
                    </form>
                    <div class="mt-4 space-y-2 max-h-64 overflow-y-auto">
                ...
                <div class="grid md:grid-cols-2 gap-6">
                    <div class="space-y-4">
                        @foreach($invitation->bankAccounts as $acc)
                            <div class="bg-white/10 rounded-2xl p-5">
                                ...
                                <button type="button" onclick="copyText('{{ $acc->account_number }}')"
                                    class="mt-3 text-sm bg-white/20 hover:bg-white/30 px-4 py-2 rounded-lg">Salin
                                    Rekening</button>
                            </div>
                        @endforeach
                    </div>
                    <div class="bg-white/10 rounded-2xl p-5">
                        ...
                        <p class="text-sm opacity-90 leading-relaxed">
                            {{ $invitation->gift_address ?: 'Alamat pengiriman hadiah belum diisi.' }}</p>
                        @if($invitation->gift_address)
                            <button type="button" onclick="copyText('{{ addslashes($invitation->gift_address) }}')"
                                class="mt-4 text-sm bg-white/20 hover:bg-white/30 px-4 py-2 rounded-lg">Salin
                                Alamat</button>
                        @endif
                    </div>
                </div>
        ...
        <section class="max-w-4xl mx-auto px-6 pb-20 text-center">
            <article class="glass-card rounded-3xl p-8">
                ...
                <p class="opacity-90">
                    {{ $invitation->closing_text ?: 'Merupakan suatu kehormatan bagi kami jika Anda berkenan hadir dan memberikan doa restu.' }}
                </p>
    ...
    <div id="lightboxOverlay" class="hidden fixed inset-0 z-[80] bg-black/90 p-4">
        <button type="button" id="lightboxPrev"
            class="absolute left-4 top-1/2 -translate-y-1/2 text-white text-2xl leading-none">&#8249;</button>
        <button type="button" id="lightboxNext"
            class="absolute right-4 top-1/2 -translate-y-1/2 text-white text-2xl leading-none">&#8250;</button>
